...
While the targets provided represent commonly observed fusions and clinically relevant variants this list is not complete. ALL is a highly heterogenous disease and a large number of rare fusions or novel fusions have been identified and implicated in disease development and progression. Furthermore, research is regularly identifying additional clinically relevant SNVs for this disease. To enable users to generate their own list of desired fusion/SNV targets we have generated two scripts for producing user defined fusions and SNVs. This script requires samtools v1.9 or later as well as Ensembl gtf file and associated reference genome.

If samtools is not already available it can be added to the km conda environment by first activating the km environment and then installing samtools via conda:
