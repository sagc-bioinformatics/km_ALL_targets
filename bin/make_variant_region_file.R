...
for(i in 1:nrow(target_file)){
  ...
  first_target_AA <- target_file$X2[i] - 11
  final_target_AA <- target_file$X3[i] + 11
  ...
  # are there multiple rows for a single transcript_name? if so target spans exons.
  ...
  single_exon_target_info <- exons_target_AA %>% 
    dplyr::add_count(transcript_name) %>% 
    filter(n == 1) %>% 
    dplyr::select(seqnames,start,end,width,strand,phase,
                  gene_name,transcript_name,transcript_name,transcript_exon,
  ...
  multi_exon_target_info <- exons_target_AA %>% 
    ...
    filter(n > 1) %>% 
    dplyr::select(seqnames,start,end,width,strand,phase,
                  gene_name,transcript_name,transcript_name,
                  remainder,no_AA_in_exon,AA_start,AA_end,extra_bases)
      ...
      if(strand == "+") {
        ...
        if(first_target_AA == single_exon_target_info$AA_start[j] & single_exon_target_info$remainder[j] == 0) {
          ...
          single_exon_targets[j,] <- single_exon_target_info[j,] %>% 
            ...
            # select require cols
            dplyr::select(gene_name, transcript_name, strand, target_range1, target_range2) %>% 
          ...
          extra_target <- filter(CDS_info, AA_end == (first_target_AA - 1), transcript_name == single_exon_target_info$transcript_name[j]) %>% 
            mutate(end_range1 = end, 
                   start_range1 = end - extra_bases + 1, 
                   target_range1 = paste(seqnames, ":", start_range1, "-", end_range1, sep = "")) %>% 
            dplyr::select(transcript_name, target_range1)
          ...
          single_exon_targets[j,] <- single_exon_target_info[j,] %>% 
            ...
            mutate(first_aa_pos = paste(first_target_AA),
                   final_aa_pos = paste(final_target_AA), 
                   target_aa_pos = ifelse(target_file$X2[i] == target_file$X3[i], paste(target_file$X2[i]), paste(target_file$X2[i],"-",target_file$X3[i],sep = "")))
  ...
  if(nrow(multi_exon_target_info) > 0){
    ...
    coalesce_all_columns <- function(df) {
      ...
      tibble(
        ...
        transcript_name = df$transcript_name[1],
        strand = df$strand[1],
    ...
    if(strand == "+") {
      ...
      multi_exon_targets <- multi_exon_target_info %>% 
        ...
        # collapse columns
        dplyr::select(gene_name,transcript_name,strand,seqnames,start_range1,end_range1,start_range2,end_range2) %>% 
        ...
        dplyr::select(gene_name,transcript_name,strand,
